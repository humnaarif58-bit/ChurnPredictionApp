...
st.markdown(
    """
    <h1 style='text-align:center; color:#2E8B57; font-size:36px; margin-bottom:0;'>🧑 Employee Churn Prediction</h1>
    <p style='text-align:center; color:grey; font-size:16px; margin-top:4px;'>Predict if an employee is likely to leave based on inputs</p>
    """,
...
# ---------------- LEFT COLUMN ----------------
with col1:
    st.markdown("<h3 style='color:#2E8B57;'>🧩 Input Features</h3>", unsafe_allow_html=True)
    st.markdown("<p style='color:grey; font-size:13px;'>Please fill in all fields below:</p>", unsafe_allow_html=True)
...
with col2:
    ...
    if predict_btn:
        ...
        st.markdown(
            f"""
            <div style="background-color:{color}; padding:18px; border-radius:10px; text-align:center; color:white; margin-top:20px;">
                <h2 style='font-size:24px; margin:0;'>Prediction: {prediction}</h2>
                <p style='font-size:16px; margin-top:6px;'>Employee is <b>{prediction_translate}</b> to churn.</p>
            </div>
            """,
            unsafe_allow_html=True
        )
    ...
    st.markdown(
        "<p style='text-align:center; font-size:14px; color:grey;'>Developed by <b>Humna Arif</b></p>",
        unsafe_allow_html=True
    )
